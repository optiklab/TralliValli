...
test.describe('Group Conversations', () => {
  test('should create a new group', async ({ page }) => {
    ...
    const groupOption = page.locator(
      'text=/group|create group|new group/i, button:has-text("Group")'
    ).first();
    ...
    const memberSearch = page.locator('input[placeholder*="search" i], input[placeholder*="member" i]');
  ...
  test('should add member to existing group', async ({ page }) => {
    ...
    const groupConversation = page.locator(
      '[role="list"] > *, .conversation-item, .group-item'
    ).first();
    ...
    if (await settingsButton.count() > 0) {
      ...
      const addMemberButton = page.locator(
        'button:has-text("Add"), button:has-text("Invite"), text=/add.*member/i'
      ).first();
