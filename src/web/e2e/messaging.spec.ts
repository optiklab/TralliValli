...
test.describe('Messaging - Direct Conversations', () => {
  test('should create a new direct conversation', async ({ page }) => {
    ...
    const newConversationBtn = page
      .locator('button:has-text("New"), button:has-text("Create"), button:has-text("Start Chat")')
      .first();

    await newConversationBtn.click();

    // Should show conversation creation dialog/modal
    await expect(page.locator('text=/new conversation|create chat|direct message/i')).toBeVisible({
      timeout: 5000,
    });
    ...
    const searchInput = page
      .locator('input[placeholder*="search" i], input[placeholder*="user" i]')
      .first();
...
test.describe('Messaging - Message Features', () => {
  test('should support emoji in messages', async ({ page }) => {
    ...
    const emojiButton = page
      .locator('button[aria-label*="emoji" i], button:has-text("😀")')
      .first();
